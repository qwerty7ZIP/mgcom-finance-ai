...
- `GET /api/data` — получение табличных данных из БД Supabase.
...
В текущем MVP данные берутся из Supabase: маршрут использует `getDbTable(table)`, который обращается к таблицам `clients`, `contacts`, `tenders` и приводит результат к формату `{ columns, rows }`.
