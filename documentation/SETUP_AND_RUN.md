...
- `xlsx` — библиотека для работы с Excel (пока используется частично);
...
## 6. Работа с исходными данными (Excel)

В проекте уже есть примерные файлы в директории `data-files/`:

- `NewBiz - Все тендеры.xlsx`
- `клиенты-ai.xlsx`
- `Контакты-ai.xlsx`

На текущем этапе (MVP UI) они **не подключены к рантайму** — данные для таблицы берутся из демо‑функции `getDemoTable`. Подключение реальных файлов планируется:

- через использование библиотеки `xlsx` или `exceljs` в серверных роутов Next.js;
- с последующим кэшированием/сохранением в БД.

Подробности см. в `PLAN.md` и `DATA_MODEL.md` (после заполнения).
