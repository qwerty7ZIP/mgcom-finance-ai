...
- **Серверные роуты** (`/api/chat`, `/api/data`) реализуют:
  ...
  - выдачу табличных данных (пока демо‑слой, далее — реальные Excel‑данные).
- **Слой данных** сейчас — демо‑функция `getDemoTable`, в дальнейшем будет заменён на чтение Excel и/или БД.
...
- `web/` — приложение Next.js (MVP интерфейса и API).
- `documentation/` — документация проекта (вы читаете один из файлов здесь).
- `data-files/` — исходные Excel‑файлы (пока не интегрированы в рантайм, но описаны в `PLAN.md`).
...
- `lib/`
  - `demoTable.ts` — генерация демо‑таблицы с колонками и строками.
...
- Серверный компонент `Home`:
  - вызывает `getDemoTable()` для загрузки стартовых колонок и строк;
